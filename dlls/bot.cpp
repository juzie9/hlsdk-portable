...
char gina_sounds[][30] = { GI_SND1, GI_SND2, GI_SND3, GI_SND4, GI_SND5 };
char colette_sounds[][30] = { CO_SND1, CO_SND2, CO_SND3, CO_SND4, CO_SND5 };
...
void CBot::Spawn( )
{
   ...
   bot_model = 0;
   if ((strcmp( model_name, "ginacol" ) == 0) ||
       (strcmp( model_name, "player" ) == 0))
   {
	  // Vyacheslav Dzhura: TODO somehow need to know DecayId in here
	   if ( m_iDecayId == 1)
		   bot_model = MODEL_GINA; 
	   else
		   bot_model = MODEL_COLETTE;
   }
   else if (strcmp( model_name, "dm_gina") == 0)
   {
      bot_model = MODEL_GINA;
   }
   else if (strcmp( model_name, "dm_colette") == 0)
   {
      bot_model = MODEL_COLETTE;
   }
                      ...
                      float flDamage, int bitsDamageType )
{
   ...
   if (
	 // Vyacheslav Dzhura: commented out for Decay
	 //  (bot_model != 0) && (pAttacker->IsNetClient()) &&
	  ...
	  )
   {
      ...
      if (distance <= 400)
      {
         ...
         if (RANDOM_LONG(1, 100) <= 50)
         {
			...
			// Vyacheslav Dzhura TODO: put Gina\Colette sounds here 
            if (bot_model == MODEL_GINA)
               strcpy( sound, gina_sounds[RANDOM_LONG(0,4)] );
            else if (bot_model == MODEL_COLETTE)
               strcpy( sound, colette_sounds[RANDOM_LONG(0,4)] );
         ...
         else
         {
            ...
            if (strcmp(item_name, STRING(tr.pHit->v.classname)) == 0)
            {
               ...
               if ( strcmp("func_healthcharger", item_name) == 0 ||
				    strcmp("item_healthcharger", item_name) == 0 )
               {
               ...
               // check if entity is wall mounted HEV charger...
						 ...
						 strcmp("item_recharge", item_name) == 0 )
               {
                  // check if the bot can use this item and
                  // check if the recharger is ready to use (has power left)...
...
void CBot::BotThink( void )
{
   ...
   {
	   ALERT( at_console, "Bot has enemy\n" );
       //ALERT( at_console, "Bot has enemy %s\n", STRING(pBotEnemy->pev->classname) );
   } else
	   ALERT( at_console, "Bot has NO ENEMY\n" );
	*/
   ...
   */
   ...
   else  // else handle movement related actions...
   {
	  ...
	  // Vyacheslav Dzhura: IMPORTNANT - DECAY'S BOT - FIND ENEMY
      if (f_botdontshoot == 0)  // is bot targeting turned on?
         pBotEnemy = BotFindEnemy( ); // leave ";" to disable bot AI
      else 
